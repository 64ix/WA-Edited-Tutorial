...
const NOTE_DETAILS = [
  { note: "C", key: "W", frequency: 261.626, active: false },
  ...
  { note: "B", key: ",", frequency: 493.883, active: false }
]

document.addEventListener('keydown', e => {
  if(e.repeat) return
  //? get the individual note that we are playing based on the key we pressed
  const keyBoardKey = e.key.toUpperCase()
  let noteKeyDetails;
  NOTE_DETAILS.forEach((n) => {
    if(keyBoardKey === n.key){
      return noteKeyDetails = n
    }
  })
  if (noteKeyDetails == null) return
  noteKeyDetails.active = true
  playNotes()
})
